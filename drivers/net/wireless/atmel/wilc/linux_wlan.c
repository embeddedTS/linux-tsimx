...
static int wilc_wlan_initialize(struct net_device *dev, struct wilc_vif *vif)
{
	...
	if (!wl->initialized) {
		...
		if (ret < 0) {
			wilc_wlan_power(wl, 0);
			wilc_wlan_power(wl, 1);
			ret = wilc_wlan_init(dev);
			if (ret < 0) {
...
void wilc_wlan_power(struct wilc *wilc, int power)
{
	if (gpio_request(wilc->gpio_chip_en, "CHIP_EN") == 0 &&
		...
		if (power) {
			gpio_set_value_cansleep(wilc->gpio_chip_en, 1);
			mdelay(5);
			gpio_set_value_cansleep(wilc->gpio_reset, 1);
		} else {
			gpio_set_value_cansleep(wilc->gpio_reset, 0);
			gpio_set_value_cansleep(wilc->gpio_chip_en, 0);
		}
