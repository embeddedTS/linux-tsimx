...
static int wilc_bus_probe(struct spi_device *spi)
{
	...
	/** If the device node contains an "interrupts" property, use that.
	Otherwise, if device node contains a "wilc3000,irq-gpios" property,
	use that. Otherwise, return an error. */

	irq = irq_of_parse_and_map(node, 0);
	if (irq > 0) {
		pr_info("wilc1000 probe, irq : %d\n", irq);
		irq_gpio = -irq;	// negative, so that wilc_netdev_init can use it */
	} else {
