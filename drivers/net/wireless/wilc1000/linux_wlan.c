...
static int init_irq(struct net_device *dev)
{
	...
	if (wl->gpio < 0) {
		...
		netdev_info(dev, "%s, IRQ using GPIO %d)\n", __func__, wl->gpio);
...
static void wilc_wlan_power(int power)
{
	...
	if ((chip_en_gpio = of_get_named_gpio(node, "wilc3000,chip-en-gpios", 0)) < 0) {
		printk("Failed to get chip EN GPIO\n");
		return;
	}


	if (gpio_request(chip_en_gpio, "CHIP_EN") == 0 &&
		 gpio_request(reset_gpio, "RESET") == 0) {
		...
		gpio_free(reset_gpio);
		 ...
		 printk("unable to request gpios for wilc_spi\n");
		 gpio_free(chip_en_gpio);
		 gpio_free(reset_gpio);
	}
}
